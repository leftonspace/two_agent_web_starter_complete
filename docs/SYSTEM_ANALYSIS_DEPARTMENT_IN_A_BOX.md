...
| R3 | **LLM API Timeout Without Fallback** | Reliability | CONFIRMED | MEDIUM | llm.py._post() retries 3 times on failure, then returns timeout stub with `timeout=True` (line 1840). Orchestrator doesn't have documented fallback logic - likely treats timeout as failure. **FIX**: Add graceful degradation. On timeout, retry with cheaper/faster model (e.g., gpt-4o → gpt-4o-mini). Add `llm_fallback_model` config. Log timeout events to analytics for monitoring. |
...
| A3 | **Hard-Coded Model Names** | Architecture | CONFIRMED | MEDIUM | config.py ModelDefaults uses hard-coded model strings like "gpt-4o-mini" (line 754-759). When OpenAI deprecates models, requires code changes. **FIX**: Move to external model registry JSON file (`models.json`) with versioning. Load registry at runtime. Add `model_aliases` to map logical names (e.g., "manager_default") to specific models. Update via config, not code. |
