(() => {
  ...
  // FAQ helpers for deep-linking
  function openFaqByButton(btn) {
    if (!btn) return;
  ...
  function ensureFaqVisibilityForElement(target) {
    if (!target) return;
    if (target.classList?.contains('faq-panel')) {
      const labelledby = target.getAttribute('aria-labelledby');
      const btn = labelledby ? document.getElementById(labelledby) : null;
      if (btn && btn.getAttribute('aria-expanded') !== 'true') openFaqByButton(btn);
    } else if (target.classList?.contains('faq-question')) {
      if (target.getAttribute('aria-expanded') !== 'true') openFaqByButton(target);
    }
  }
  ...
  document.addEventListener('click', (e) => {
    ...
    // If deep-linking to a FAQ panel/question, ensure it is visible first
    ensureFaqVisibilityForElement(target);

    e.preventDefault();
    scrollToTarget(target);
    // Mirror native behavior: update hash without immediate browser jump
    try { history.pushState(null, '', '#' + id); } catch (_) {}
  });

  // Adjust anchor position on initial load if URL has a hash
  function handleInitialOrHashChange() {
  ...
  if (location.hash && location.hash.length > 1) {
    handleInitialOrHashChange();
  }
  window.addEventListener('hashchange', handleInitialOrHashChange);
